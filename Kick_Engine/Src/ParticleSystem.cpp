...
void ParticleSystem::Update(float deltaTime)
{
    ...
    if (hasUpdate)
    {
        ...
        {
            float distSqA = Kick_Math::MagnitudeSqr(mParticles[a]->GetTransform().position - mCamera->GetPosition());
            float distSqB = Kick_Math::MagnitudeSqr(mParticles[b]->GetTransform().position - mCamera->GetPosition());
...
void ParticleSystem::SetPosition(const Kick_Math::Vector3& position)
{
...
void ParticleSystem::DebugUI()
{
    if (ImGui::CollapsingHeader("ParticleSystem", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        {
            mInfo.spawnDirection = Kick_Math::Normalize(mInfo.spawnDirection);
        }
...
void ParticleSystem::SpawnParticle()
{
    ...
    // gets float from 0-1
    Kick_Math::Vector3 spawnDirection = mInfo.spawnDirection;
    if (mInfo.maxSpawnAngle > 0.0f)
    {
        ...
        if (abs(Kick_Math::Dot(mInfo.spawnDirection, Vector3::YAxis)) > 0.99f)
        {
