...
void GameState::Initialize()
{
	...
	CreatePlanet(0.05, 6.3, 15.945, 15.945, L"../../Assets/Images/planets/titan.jpg");
	...
	CreatePlanet(0.1, 4.6, 27.3, 27.3, L"../../Assets/Images/planets/moon.jpg");
...
#pragma endregion
	...
	int earthIndex = 3;
	int moonIndex = planetCount - 1; // Assuming the Moon is the last added celestial body

	// Update Moon's orbital angle based on its orbit time
	mInfo[moonIndex]->orbitAngle += deltaTime * 2.0 * Math::Constants::Pi / mInfo[moonIndex]->orbitTime;
	if (mInfo[moonIndex]->orbitAngle >= 2.0 * Math::Constants::Pi)
	{
		mInfo[moonIndex]->orbitAngle -= 2.0 * Math::Constants::Pi;
	}

	// Calculate Moon's position relative to Earth
	float moonDistanceX = mInfo[moonIndex]->orbitRadius * cos(mInfo[moonIndex]->orbitAngle);
	float moonDistanceZ = mInfo[moonIndex]->orbitRadius * sin(mInfo[moonIndex]->orbitAngle);

	// Update Moon's position based on Earth's position plus the calculated distance
	mInfo[moonIndex]->position.x = mInfo[earthIndex]->position.x + moonDistanceX;
	mInfo[moonIndex]->position.z = mInfo[earthIndex]->position.z + moonDistanceZ;
	int saturnIndex = 6;
	int titanIndex = planetCount - 1; // Titan is the last added celestial body

	// Update Titan's orbital angle
	mInfo[titanIndex]->orbitAngle += deltaTime * 2.0 * Math::Constants::Pi / mInfo[titanIndex]->orbitTime;
	if (mInfo[titanIndex]->orbitAngle >= 2.0 * Math::Constants::Pi)
	{
		mInfo[titanIndex]->orbitAngle -= 2.0 * Math::Constants::Pi;
	}

	// Calculate Titan's position relative to Saturn
	float titanDistanceX = mInfo[titanIndex]->orbitRadius * cos(mInfo[titanIndex]->orbitAngle);
	float titanDistanceZ = mInfo[titanIndex]->orbitRadius * sin(mInfo[titanIndex]->orbitAngle);

	// Update Titan's position based on Saturn's position plus the calculated distance
	mInfo[titanIndex]->position.x = mInfo[saturnIndex]->position.x + titanDistanceX;
	mInfo[titanIndex]->position.z = mInfo[saturnIndex]->position.z + titanDistanceZ;
}
